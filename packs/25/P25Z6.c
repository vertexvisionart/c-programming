...
static void heap_insert(int fd, long long *size, int value) {
    long long pos = *size;
    ++(*size);
    ...
    while (pos > 0) {
        ...
        int parent_value = 0;
        read_int_at(fd, parent, &parent_value);
        if (parent_value <= value) {
            break;
        }
        write_int_at(fd, pos, parent_value);
        pos = parent;
    }
    write_int_at(fd, pos, value);
}

static int heap_extract_min(int fd, long long *size) {
    int minimum = 0;
    int last = 0;
    read_int_at(fd, 0, &minimum);
    read_int_at(fd, *size - 1, &last);
    --(*size);
    ...
    while (1) {
        ...
        read_int_at(fd, left, &best_value);
        if (right < *size) {
            int right_value = 0;
            read_int_at(fd, right, &right_value);
        ...
        write_int_at(fd, pos, best_value);
        pos = best;
    }

    write_int_at(fd, pos, last);
    return minimum;
}
...
int main(void) {
    ...
    const char *temp_name = "p25z6_heap.bin";
    int heap_file = open(temp_name, O_RDWR | O_CREAT | O_TRUNC, 0600);
    if (heap_file < 0) {
        return 1;
    }
    ...
    for (long long i = 0; i < q && read_token(&in, token, sizeof(token)); ++i) {
        if (token[0] == '+') {
            ...
            heap_insert(heap_file, &size, (int)value);
        } else if (token[0] == '-') {
            if (size > 0) {
                int value = heap_extract_min(heap_file, &size);
                write_ll(&out, value);
                write_char(&out, '\n');
            }
        } else {
